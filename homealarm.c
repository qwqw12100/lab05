...
void toalarmoff(){
 ...
 time_t timestar1s;
 time_t t1;
 timestar1s = time(NULL);
 while(difftime(time(NULL), timestar1s) < 2.00 );
...
void toalarming(){
 ...
 time_t t1;
 timestar1s = time(NULL);
 while(difftime(time(NULL), timestar1s) < 1.00 );
...
void toalarmsounding(){
	time_t timestar;
	ifttt("https://maker.ifttt.com/trigger/alarm_triggered/with/key/dImUb3e_p6ayih5X2iy8RE", "my1", "my 2", "my 33333");
	while(1){
    timestar = time(NULL);
